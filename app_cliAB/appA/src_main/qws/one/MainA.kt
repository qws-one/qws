...
object MainA {
    ...
    val poemsStore by lazy {
        ...
        StaticStoreContainer.init(dbFolder = Conf.dBxd(), environmentName = "poemsStore")
...
//        println(" "+Empty.word.text)
        ...
        val user = poemsStore.transactional {/* ... */}

        poemsStore.transactional(readonly = true) {
            for (poem in Poem.all().asIterable()) {
                println("CliA0.main ${poem.title}")
                for (paragraph in poem.paragraphs.asIterable()) {
                    for (word in paragraph.words.asIterable()) {
                        println(" ${word.text}")
                    }
                }
            }

...
${"tools/toolPlainIdeListener/src/PlainIdeListener.kt".content()}

PlainIdeListener.start(bindings)
            """
            //java.io.File("$projectBasePath/tools/toolPlainIdeListener/tmp/entryPointPlainIdeScript.kts").writeText(scriptStr)
            val res = entryPointScriptEngine.eval(scriptStr)
            ide.print(res)
        } catch (e: Exception) {
            e.printStackTrace()
            ide.error(e)

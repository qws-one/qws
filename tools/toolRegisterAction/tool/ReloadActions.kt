...
res
    """
            java.io.File(moduleDir, "tmp2/reload-${scriptFile.name}.kts")
    ...
    private fun reloadActions(scriptSha1: String, project: IdeProject, outputPanel: Lib.OutputPanel) {
        ...
        java.io.File("$basePath/tools/toolRegisterAction/src/").listFiles()?.forEach {
            go(it.absolutePath, outputPanel)
        }
        val scriptFile = java.io.File("$basePath/tools/toolRegisterAction/tool/ReloadActions.kt")
